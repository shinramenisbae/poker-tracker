...
function playerHelp(settings, uiBase) {
  return [
    ...
    'Check who\'s still outstanding with `/unpaid` — in a session thread for that',
    'session, anywhere else for everyone\'s debts across all sessions.',
    '',
    `**4. Reminders** — every day at ${scheduleOf(settings)} I ping anyone who still`,
    'owes the bank player, until everyone\'s marked paid.',
    '',
    '**Rake** — `/rake balance` shows the pile and who\'s holding it. Spent some on',
    'the group? `/rake spend amount:120 note:new chips`. Handed it on? `/rake give`.',
    '',
